...
void main() {
  group('SliverListBuilderBuilder', () {
    const b = SliverListBuilderBuilder();
      ...
      expect(
        () => b.build(
          const ResolvedArguments(
            named: {'itemCount': 3, 'itemBuilder': 7},
          ),
          testContext(),
        ),
        throwsA(isA<ArgumentException>()),
    ...
    testWidgets('happy path renders sliver items', (tester) async {
      final sliver = b.build(
        ResolvedArguments(
          named: {
            'itemCount': 3,
            'itemBuilder': _closureOf("(c, i) => Text('row')"),
          },
        ),
        testContext(),
      );
