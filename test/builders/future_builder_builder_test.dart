...
void main() {
  group('FutureBuilderBuilder', () {
    const b = FutureBuilderBuilder();

    test('typeName is "FutureBuilder"', () {
      expect(b.typeName, 'FutureBuilder');
    });
    ...
    test('wrong-arity builder throws ArgumentException', () {
      expect(
        () => b.build(
          ResolvedArguments(
            named: {'builder': _closureOf("(snap) => Text('x')")},
          ),
          testContext(),
        ),
        throwsA(isA<ArgumentException>()),
    ...
    testWidgets('happy path renders initialData via closure', (tester) async {
      final widget = b.build(
        ResolvedArguments(
          named: {
            'initialData': 'hi',
            'builder': _closureOf("(c, s) => Text('shown')"),
          },
        ),
        testContext(),
      );
    ...
    testWidgets('future resolves and drives rebuild', (tester) async {
      final widget = b.build(
        ResolvedArguments(
          named: {
            'future': Future<Object?>.value('ok'),
            'builder': _closureOf("(c, s) => Text('rebuilt')"),
          },
        ),
        testContext(),
      );
