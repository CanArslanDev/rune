...
Current release: **`v0.11.0`**. Source-level state via the new `StatefulBuilder` widget plus `RuneState`: `initial` map seeds mutable state, `state.counter = value` writes through property-access assignment, and `setState(() { ... })` wraps a batch of mutations the Flutter-idiomatic way. Phase C + Phase D of the v1.0.0 roadmap bundled.
...
| Value ctors           | `EdgeInsets.all/symmetric/only/fromLTRB/zero`, `Color(hex)`, `TextStyle(...)`, `BorderRadius.circular(n)`, `BoxDecoration(...)`, `Image.network(url)`, `Image.asset(path)`, `Duration(...)`, `BottomNavigationBarItem(...)`, `Transform.scale/.rotate`, `Transform.translate`, `Transform.flip`, `Offset(dx, dy)`, `BoxConstraints(...)`, `NavigationDestination(...)`, `NavigationRailDestination(...)`                 |
| Constants             | `Colors.*`, `MainAxisAlignment.*`, `CrossAxisAlignment.*`, `MainAxisSize.*`, `TextAlign.*`, `TextOverflow.*`, `Alignment.*`, `BoxFit.*`, `StackFit.*`, `Axis.*`, `FontWeight.*`, `BoxShape.*`, `FlexFit.*`, `BottomNavigationBarType.*`, `CrossFadeState.*`, `Clip.*`, `DecorationPosition.*`, `ListTileControlAffinity.*`, `NavigationRailLabelType.*`, `Curves.linear/easeIn/easeOut/easeInOut/bounce*/elastic*/fastOutSlowIn`, ~60 common `Icons.*` |
...
| Stateful source       | `StatefulBuilder(initial: {...}, builder: (state) => ...)` produces source-level state; `state.key` reads, `state.key = value` assigns, `setState(() { ... })` wraps a batch of mutations.                 |
