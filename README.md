...
Upgrading from an earlier release? See [`MIGRATION.md`](MIGRATION.md) for version-to-version notes. API reference documentation is generated by `dart doc` from the source; pub.dev automatically builds and hosts it on each published release.

## Detailed documentation

This README is the 5-minute tour. For deeper material, see [`guides/`](guides/):

- [`guides/getting-started.md`](guides/getting-started.md): derin quickstart, data binding, events, error handling.
- [`guides/source-syntax.md`](guides/source-syntax.md): desteklenen Dart syntax'ının tam referansı.
- [`guides/cookbook.md`](guides/cookbook.md): copy-paste recipes.
- [`guides/bridges.md`](guides/bridges.md): 5 sibling bridge paketinin setup ve kullanımı.
- [`guides/devtools.md`](guides/devtools.md): Flutter DevTools extension.
- [`guides/troubleshooting.md`](guides/troubleshooting.md): common errors, exception hierarchy.
...
Current release: **v1.21.0**. Borders, gradients, and time breadth. `Border.all`, `Border.symmetric`, and standalone `BorderSide` close the decoration-border gap so source can write `BoxDecoration(border: Border.all(color: Colors.red, width: 2))`. Three new gradient value builders (`LinearGradient`, `RadialGradient`, `SweepGradient`) plug straight into `BoxDecoration(gradient: ...)` with stops lists that auto-coerce `num` to `double`. `BorderStyle` and `TileMode` enums land as constants. On the runtime side, `Duration` exposes its six unit projections (`.inDays`, `.inHours`, `.inMinutes`, `.inSeconds`, `.inMilliseconds`, `.inMicroseconds`), and `DateTime` gains field accessors (`.year`, `.month`, `.day`, `.hour`, `.minute`, `.second`, `.weekday`, ...) plus five method dispatchers (`.isBefore`, `.isAfter`, `.difference`, `.add`, `.subtract`). Previous: **v1.20.0** geometry constructors, color breadth, state-management trio (see CHANGELOG for the full bundle).
...
| Built-in properties   | `.length`, `.isEmpty`, `.isNotEmpty`, `.first`, `.last` on lists; `.length`, `.isEmpty`, `.isNotEmpty` on strings; `.length`, `.isEmpty`, `.isNotEmpty`, `.keys`, `.values` on maps; `.hasData`/`.data`/`.hasError`/`.error`/`.connectionState` on `AsyncSnapshot`; `.maxWidth`/`.minWidth`/`.maxHeight`/`.minHeight`/`.biggest`/`.smallest` on `BoxConstraints`; `.alpha`/`.red`/`.green`/`.blue`/`.opacity`/`.value` on `Color`; `.inDays`/`.inHours`/`.inMinutes`/`.inSeconds`/`.inMilliseconds`/`.inMicroseconds` on `Duration`; `.year`/`.month`/`.day`/`.hour`/`.minute`/`.second`/`.weekday`/`.millisecondsSinceEpoch` on `DateTime`; ThemeData, ColorScheme, TextTheme, MediaQueryData, Size, EdgeInsets property access (see CHANGELOG)                                 |
| Built-in methods      | `toString()` (any); `toUpperCase/toLowerCase/trim/contains/startsWith/endsWith/split/substring/replaceAll` on strings; `contains/indexOf/join` on lists plus closure-accepting `map/where/any/every/firstWhere/forEach/fold/reduce`; `containsKey/containsValue` on maps; `abs/round/floor/ceil/toInt/toDouble` on num; `isBefore/isAfter/difference/add/subtract` on `DateTime`  |
