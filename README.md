...
| Built-in properties   | `.length`, `.isEmpty`, `.isNotEmpty`, `.first`, `.last` on lists; `.length`, `.isEmpty`, `.isNotEmpty` on strings; `.length`, `.isEmpty`, `.isNotEmpty`, `.keys`, `.values` on maps; `.hasData`/`.data`/`.hasError`/`.error`/`.connectionState` on `AsyncSnapshot`; `.maxWidth`/`.minWidth`/`.maxHeight`/`.minHeight`/`.biggest`/`.smallest` on `BoxConstraints`                                 |
...
| Imperative bridges    | `showDialog(builder: ...)`, `showModalBottomSheet(builder: ...)`, `showSnackBar(snackBar)`, `Navigator.pop(result?)`. All route through `RuneContext.flutterContext`. |
